...
class Play:
    def __init__(self, env, agent, env_name, max_episode=1):
        ...
        self.VideoWriter = cv2.VideoWriter("./Movie/" + env_name + ".mp4", self.fourcc, 50.0, (250, 250)) #modified 22.04.30
    ...
    def evaluate(self):
        ...
        for _ in range(self.max_episode):
            ...
            for _ in range(self.env._max_episode_steps):
                ...
                cv2.imshow("env", I) #if visualize on
                cv2.waitKey(10)      #if visualize on
            print(f"episode reward:{episode_reward:3.3f}") # for print per 1episode
        self.env.close()
        self.VideoWriter.release()
        cv2.destroyAllWindows()
        return episode_reward  #added
